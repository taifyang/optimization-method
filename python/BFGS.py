...
def BFGS_x(f, x0, max_iter, epsilon):
    '''
...
def BFGS_x0x1(f, X0, max_iter, epsilon):
    '''
    description:    BFGS算法（多元变量）
    param f         要求极值的函数
    param X0        初始位置
    param max_iter  最大迭代次数
    param epsilon   相邻两次迭代的改变量
...
if __name__ == '__main__':
    ...
    x0 = sympy.symbols("x0")
    x1 = sympy.symbols("x1")
    result = BFGS_x(x**4-4*x, 10, 100, 1e-5)
    print('最终迭代位置：%.5f' % result)
    result = BFGS_x0x1((x0-1)**2+(x1-1)**4, [10, 10], 100, 1e-5)
