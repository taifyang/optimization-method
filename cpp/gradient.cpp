...
/**
 * @description: 	梯度下降算法（一元变量）
 * @param x0		初始位置
 * @param max_iter	最大迭代次数
 * @param epsilon	相邻两次迭代的改变量
 * @return 			结束位置
 */
double gradient_x(double x0, int max_iter, double epsilon)
{
	int i = 0;
	double alpha = 0.001;
	double beta = 0.5;
 ...
 * @return 			结束位置
 */
std::vector<double> gradient_x0x1(std::vector<double> X0, int max_iter, double epsilon)
{
	int i = 0;
	double alpha = 0.001;
	double beta = 0.5;
...
{
	double result = gradient_x(10, 100, 1e-5);
	std::cout << "最终迭代位置：" << result << std::endl;

	std::vector<double> results = gradient_x0x1({ 10,10 }, 100, 1e-5);
