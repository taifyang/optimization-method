 ...
 * @return 			结束位置
 */
double Broyden_x(double x0, int max_iter, double epsilon)
{
	int i = 0;
	double beta = 0.5;
	double delta = 0.25;
	...
	while (i < max_iter)
	{
		...
		float phik = 0.5;
		float vk = sqrt(yk*Hk*yk)*(sk / (yk*sk) - (Hk*yk) / (yk*Hk*yk));
 ...
 * @return 			结束位置
 */
std::vector<double> Broyden_x0x1(std::vector<double> X0, int max_iter, double epsilon)
{
	int i = 0;
	double beta = 0.5;
	double delta = 0.25;
	...
	while (i < max_iter)
	{
		...
		float phik = 0.5;
		Eigen::Vector2d vk = sqrt(yk.transpose()*Hk*yk)*(sk / (sk.transpose()*yk)- (Hk*yk) / (yk.transpose()*Hk*yk));
		Hk = Hk - (Hk*yk*yk.transpose()*Hk) / (yk.transpose()*Hk*yk) + (sk*sk.transpose()) / (sk.transpose()*yk) + phik*vk*vk.transpose();
...
{
	double result = Broyden_x(10, 100, 1e-5);
	std::cout << "最终迭代位置：" << result << std::endl;

	std::vector<double> results = Broyden_x0x1({ 10,10 }, 100, 1e-5);
